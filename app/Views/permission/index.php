            ...
            <div class="row align-items-end">
                ...
                <div class="col-md-2 mb-2 mb-md-0">
                    ...
                    <div class="d-flex" style="gap:6px;">
                        ...
                        <a href="<?= base_url('permissions') ?>" class="btn btn-outline-secondary btn-sm"><i class="fas fa-times"></i></a>
                    </div>
                </div>
    ...
    <div class="card-body p-0">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr id="row-<?= $p['id'] ?>">
                            ...
                            <td><span class="badge badge-info"><?= esc($p['module']) ?></span></td>
                            <td class="text-muted small" style="max-width:220px;"><?= esc($p['description'] ?? '-') ?></td>
                            <td class="text-center">
                                <div class="btn-group btn-group-sm">
                                    <button class="btn btn-warning" onclick="openEditModal(<?= $p['id'] ?>)" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger" onclick="hapusPermission(<?= $p['id'] ?>, '<?= esc($p['display_name']) ?>')" title="Hapus">
        ...
        <div class="px-3 py-3 border-top d-flex justify-content-between align-items-center flex-wrap">
            <small class="text-muted mb-1">
                Menampilkan <?= $offset + 1 ?>–<?= min($offset + $perPage, $total) ?> dari <?= $total ?> permission
            </small>
            <?= $pager->links() ?>
        </div>
        <?php endif; ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="c_name" class="form-control" placeholder="contoh: user.view, laporan.export">
                    <small class="text-muted">Format: <code>module.aksi</code></small>
                </div>
                <div class="form-group">
                    <label>Display Name <span class="text-danger">*</span></label>
                    <input type="text" id="c_display_name" class="form-control" placeholder="contoh: Lihat User">
                </div>
                <div class="form-group">
                    <label>Module <span class="text-danger">*</span></label>
                    <input type="text" id="c_module" class="form-control" placeholder="contoh: user, laporan, role">
                </div>
                <div class="form-group">
                    <label>Deskripsi</label>
                    <textarea id="c_description" class="form-control" rows="2" placeholder="Deskripsi singkat..."></textarea>
                </div>
            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label>Display Name <span class="text-danger">*</span></label>
                    <input type="text" id="e_display_name" class="form-control">
                </div>
                <div class="form-group">
                    <label>Module <span class="text-danger">*</span></label>
...
<!-- =========================================================
     SCRIPT — fungsi global, tidak dibungkus ready()
     ========================================================= -->
<script>
...
function doAjax(url, fd, callback) {
    ...
    .then(function(data) {
        if (data.csrf_hash) CHASH = data.csrf_hash;
        callback(null, data);
    })
...
function resetModal(id) {
    jQuery(id).removeAttr('aria-hidden').removeAttr('style').removeClass('show');
...
document.addEventListener('DOMContentLoaded', function () {
    var cName = elId('c_name');
    if (cName) cName.addEventListener('input', function () {
        this.value = this.value.toLowerCase().replace(/\s+/g, '.').replace(/[^a-z0-9._-]/g, '');
    });
    var eName = elId('e_name');
    if (eName) eName.addEventListener('input', function () {
        this.value = this.value.toLowerCase().replace(/\s+/g, '.').replace(/[^a-z0-9._-]/g, '');
    });
    var cMod = elId('c_module');
    if (cMod) cMod.addEventListener('input', function () {
        this.value = this.value.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_-]/g, '');
    });
    var eMod = elId('e_module');
    if (eMod) eMod.addEventListener('input', function () {
        this.value = this.value.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_-]/g, '');
    });
...
function openCreateModal() {
    ...
    ['c_name','c_display_name','c_module','c_description'].forEach(function(id) {
        var el = elId(id); if (el) el.value = '';
    });
    hideAlert('alert-create');
    jQuery('#modalCreate').modal('show');
}
...
function submitCreate() {
    ...
    var fd = new FormData();
    fd.append('name',         elId('c_name').value.trim());
    fd.append('display_name', elId('c_display_name').value.trim());
    fd.append('module',       elId('c_module').value.trim());
    fd.append('description',  elId('c_description').value.trim());

    doAjax(BASE + '/permissions/store', fd, function(err, data) {
        if (err) { showAlert('alert-create', 'Error: ' + err.message); setBtn('btn-create', false); return; }
        if (data.status === 'success') {
            jQuery('#modalCreate').modal('hide');
            location.reload();
        } else {
            showAlert('alert-create', data.message);
...
function openEditModal(id) {
    ...
    fetch(BASE + '/permissions/edit/' + id, {
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
    })
    .then(function(res) { return res.json(); })
    .then(function(resp) {
        ...
        var d = resp.data;
        elId('e_id').value           = d.id;
        elId('e_name').value         = d.name;
        elId('e_display_name').value = d.display_name;
        elId('e_module').value       = d.module;
        elId('e_description').value  = d.description || '';
        jQuery('#modalEdit').modal('show');
    })
    .catch(function(e) { alert('Gagal memuat data: ' + e.message); });
}
...
function submitEdit() {
    ...
    var fd = new FormData();
    fd.append('name',         elId('e_name').value.trim());
    fd.append('display_name', elId('e_display_name').value.trim());
    fd.append('module',       elId('e_module').value.trim());
    fd.append('description',  elId('e_description').value.trim());

    doAjax(BASE + '/permissions/update/' + elId('e_id').value, fd, function(err, data) {
        if (err) { showAlert('alert-edit', 'Error: ' + err.message); setBtn('btn-edit', false, 'Update'); return; }
        if (data.status === 'success') {
            jQuery('#modalEdit').modal('hide');
            location.reload();
        } else {
            showAlert('alert-edit', data.message);
...
// ── DELETE ────────────────────────────────────────────────────
function hapusPermission(id, nama) {
    if (!confirm('Hapus permission "' + nama + '"?\nPermission yang masih dipakai role tidak bisa dihapus.')) return;

    doAjax(BASE + '/permissions/delete/' + id, new FormData(), function(err, data) {
