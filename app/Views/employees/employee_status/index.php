...
<?= $this->section('content') ?>
        ...
        <main class="content">
            ...
            <div class="page-header">
                <div class="page-title">
                    ...
                    <h1>Manajemen Employee Status</h1>
                </div>
            ...
            <div class="stats-summary">
                <div class="stat-box">
                    <span class="icon">📋</span>
                    <div class="content">
                        <h4>Total Employee Status</h4>
                        <div class="value"><?= count($employeeStatuses) ?></div>
                    </div>
                </div>
                <div class="stat-box">
                    <span class="icon">✅</span>
                    <div class="content">
                        <h4>Aktif</h4>
                        <div class="value">
                            <?= count(array_filter($employeeStatuses, fn($es) => !($es['is_deleted'] === true || $es['is_deleted'] === 't' || $es['is_deleted'] === '1' || $es['is_deleted'] === 1))) ?>
                        </div>
                    </div>
                </div>
                <div class="stat-box">
            ...
            <div class="table-card">
                ...
                <div class="table-responsive">
                    <table class="employee-table">
                        ...
                        <tbody>
                            <?php if (!empty($employeeStatuses)): ?>
                                <?php foreach ($employeeStatuses as $i => $es):
                                    $isDeleted = (
                                        $es['is_deleted'] === true ||
                                        $es['is_deleted'] === 't'  ||
                                        $es['is_deleted'] === '1'  ||
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="6" style="text-align:center; padding:40px; color:#999;">
                                        Belum ada data employee status. Klik "Tambah" untuk menambahkan.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
    ...
    <!-- ═══════════════════════════════════════════════
         Add / Edit Modal
    ═══════════════════════════════════════════════ -->
    <div id="esModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.5); z-index:9999; align-items:center; justify-content:center;">
        <div class="modal-content" style="background:#fff; border-radius:12px; width:100%; max-width:520px; margin:auto;">
            <div class="modal-header">
                <h2 id="esModalTitle">➕ Tambah Employee Status</h2>
                <button class="modal-close" id="btnCloseEsModal">&times;</button>
            </div>
            <div class="modal-body">
                <form id="esForm" method="POST" action="">
                    <?= csrf_field() ?>
                    <input type="hidden" id="oldEsId" name="old_id">

                    <div class="form-group">
                        <label>ID <span class="required">*</span></label>
                        <input type="text" class="form-control" id="esId" name="id" placeholder="EMST-BDM-...">
                        <div id="idError" style="color:#dc3545; font-size:.85em; margin-top:4px; display:none;"></div>
                    </div>

                    <div class="form-group">
                        <label>Nama <span class="required">*</span></label>
                        <input type="text" class="form-control" id="esName" name="name" placeholder="contoh: Permanen, PKWT, Trainee">
                        <div id="nameError" style="color:#dc3545; font-size:.85em; margin-top:4px; display:none;"></div>
                    </div>

                    <div class="form-group">
                        <label>Deskripsi</label>
                        <textarea class="form-control" id="esDescription" name="description" rows="3" placeholder="Deskripsi employee status (opsional)"></textarea>
                    </div>
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary" id="btnCancelEsModal">Batal</button>
                <button class="btn btn-primary" id="btnSaveEsModal">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // ── Elements ──────────────────────────────────────────
        const esModal      = document.getElementById('esModal');
        const esForm       = document.getElementById('esForm');
        const esTitle      = document.getElementById('esModalTitle');
        const esId         = document.getElementById('esId');
        const oldEsId         = document.getElementById('oldEsId');
        const esName       = document.getElementById('esName');
        const esDesc       = document.getElementById('esDescription');
        ...
        document.getElementById('btnTambah').addEventListener('click', function () {
            esTitle.textContent     = '➕ Tambah Employee Status';
            oldEsId.value              = '';
            esId.value              = '';
            esName.value            = '';
            esDesc.value            = '';
            currentAliases             = [];
            renderAliasList();
            esForm.action           = BASE_URL + 'employees/employee-status/store';
        ...
        document.querySelectorAll('.btn-edit').forEach(function (btn) {
            btn.addEventListener('click', function () {
                esTitle.textContent     = '✏️ Edit Employee Status';
                oldEsId.value              = this.dataset.id;
                esId.value              = this.dataset.id;
                esName.value            = this.dataset.name;
                esDesc.value            = this.dataset.description;
                currentAliases                = JSON.parse(this.dataset.aliases || '[]');
                renderAliasList();              // ← populate the list UI
                esForm.action           = BASE_URL + 'employees/employee-status/update/' + this.dataset.id;
                nameError.style.display = 'none';
                showModal(esModal);
            });
        });
        ...
        // ── Save ─────────────────────────────────────────────
        document.getElementById('btnSaveEsModal').addEventListener('click', function () {
            ...
            esForm.appendChild(aliasInput);

            esForm.submit();
        });

        // ── Close Add/Edit Modal ──────────────────────────────
        document.getElementById('btnCloseEsModal').addEventListener('click',  function () { hideModal(esModal); });
        document.getElementById('btnCancelEsModal').addEventListener('click', function () { hideModal(esModal); });
        esModal.addEventListener('click', function (e) { if (e.target === esModal) hideModal(esModal); });

        // ── Toggle Buttons ────────────────────────────────────
        document.querySelectorAll('.btn-toggle').forEach(function (btn) {
            btn.addEventListener('click', function () {
                toggleForm.action      = BASE_URL + 'employees/employee-status/toggle/' + this.dataset.id;
                toggleName.textContent = this.dataset.name;

                if (this.dataset.action === 'disable') {
                    toggleTitle.textContent      = '⏸️ Konfirmasi Nonaktifkan';
                    toggleMessage.textContent    = 'Apakah Anda yakin ingin menonaktifkan employee status:';
                    toggleConfirm.className      = 'btn btn-danger';
                    toggleConfirm.textContent    = '⏸️ Nonaktifkan';
                } else {
                    toggleTitle.textContent      = '✅ Konfirmasi Aktifkan';
                    toggleMessage.textContent    = 'Apakah Anda yakin ingin mengaktifkan kembali employee status:';
