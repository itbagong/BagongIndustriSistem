...
<?= $this->section('content') ?>
        ...
        <main class="content">
            ...
            <div class="page-header">
                <div class="page-title">
                    ...
                    <h1>Manajemen Last Education</h1>
                </div>
                <div class="header-actions">
                    <button class="btn btn-primary" id="btnTambah">
                        <span class="btn-icon">➕</span>
                        Tambah Last Education
                    </button>
                </div>
            </div>
            ...
            <div class="stats-summary">
                <div class="stat-box">
                    <span class="icon">🎓</span>
                    <div class="content">
                        <h4>Total Last Education</h4>
                        <div class="value"><?= count($lastEducations) ?></div>
                    </div>
                </div>
                <div class="stat-box">
                    <span class="icon">✅</span>
                    <div class="content">
                        <h4>Aktif</h4>
                        <div class="value">
                            <?= count(array_filter($lastEducations, fn($es) => !($es['is_deleted'] === true || $es['is_deleted'] === 't' || $es['is_deleted'] === '1' || $es['is_deleted'] === 1))) ?>
                        </div>
                    </div>
                </div>
                <div class="stat-box">
            ...
            <div class="table-card">
                <div class="table-header">
                    <h3>Daftar Last Education</h3>
                    <div class="table-info">
                        Menampilkan <strong><?= count($lastEducations) ?></strong> dari <strong><?= $pager->getTotal() ?></strong> data
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="employee-table">
                        ...
                        <tbody>
                            <?php if (!empty($lastEducations)): ?>
                                <?php foreach ($lastEducations as $i => $es):
                                    $isDeleted = (
                                        $es['is_deleted'] === true ||
                                        $es['is_deleted'] === 't'  ||
                                        $es['is_deleted'] === '1'  ||
    ...
    <!-- ═══════════════════════════════════════════════
         Add / Edit Modal
    ═══════════════════════════════════════════════ -->
    <div id="leModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.5); z-index:9999; align-items:center; justify-content:center;">
        <div class="modal-content" style="background:#fff; border-radius:12px; width:100%; max-width:520px; margin:auto;">
            <div class="modal-header">
                <h2 id="leModalTitle">➕ Tambah Last Education</h2>
                <button class="modal-close" id="btnCloseLeModal">&times;</button>
            </div>
            <div class="modal-body">
                <form id="leForm" method="POST" action="">
                    <?= csrf_field() ?>
                    <input type="hidden" id="oldLeId" name="old_id">

                    <div class="form-group">
                        <label>ID <span class="required">*</span></label>
                        <input type="text" class="form-control" id="leId" name="id" placeholder="LTD-BDM-...">
                        <div id="idError" style="color:#dc3545; font-size:.85em; margin-top:4px; display:none;"></div>
                    </div>

                    <div class="form-group">
                        <label>Nama <span class="required">*</span></label>
                        <input type="text" class="form-control" id="leName" name="name" placeholder="contoh: SMA Sederajat, Sarjana 1...">
                        <div id="nameError" style="color:#dc3545; font-size:.85em; margin-top:4px; display:none;"></div>
                    </div>

                    <div class="form-group">
                        <label>Deskripsi</label>
                        <textarea class="form-control" id="leDescription" name="description" rows="3" placeholder="Deskripsi tingkat pendidikan (opsional)"></textarea>
                    </div>
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary" id="btnCancelLeModal">Batal</button>
                <button class="btn btn-primary" id="btnSaveLeModal">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // ── Elements ──────────────────────────────────────────
        const leModal      = document.getElementById('leModal');
        const leForm       = document.getElementById('leForm');
        const leTitle      = document.getElementById('leModalTitle');
        const leId         = document.getElementById('leId');
        const oldLeId      = document.getElementById('oldLeId');
        const leName       = document.getElementById('leName');
        const leDesc       = document.getElementById('leDescription');
        ...
        document.getElementById('btnTambah').addEventListener('click', function () {
            leTitle.textContent     = '➕ Tambah Last Education';
            oldLeId.value           = '';
            leId.value              = '';
            leName.value            = '';
            leDesc.value            = '';
            ...
            leForm.action           = BASE_URL + 'employees/last-education/store';
        ...
        document.querySelectorAll('.btn-edit').forEach(function (btn) {
            btn.addEventListener('click', function () {
                leTitle.textContent     = '✏️ Edit Last Education';
                oldLeId.value           = this.dataset.id;
                leId.value              = this.dataset.id;
                leName.value            = this.dataset.name;
                leDesc.value            = this.dataset.description;
                currentAliases                = JSON.parse(this.dataset.aliases || '[]');
                renderAliasList();              // ← populate the list UI
                leForm.action           = BASE_URL + 'employees/last-education/update/' + this.dataset.id;
                nameError.style.display = 'none';
                showModal(leModal);
            });
        });
        ...
        // ── Save ─────────────────────────────────────────────
        document.getElementById('btnSaveLeModal').addEventListener('click', function () {
            ...
            leForm.appendChild(aliasInput);

            leForm.submit();
        });

        // ── Close Add/Edit Modal ──────────────────────────────
        document.getElementById('btnCloseLeModal').addEventListener('click',  function () { hideModal(leModal); });
        document.getElementById('btnCancelLeModal').addEventListener('click', function () { hideModal(leModal); });
        leModal.addEventListener('click', function (e) { if (e.target === leModal) hideModal(leModal); });

        // ── Toggle Buttons ────────────────────────────────────
        document.querySelectorAll('.btn-toggle').forEach(function (btn) {
            btn.addEventListener('click', function () {
                toggleForm.action      = BASE_URL + 'employees/last-education/toggle/' + this.dataset.id;
