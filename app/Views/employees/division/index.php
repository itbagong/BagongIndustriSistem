...
<?= $this->section('content') ?>
            ...
            <div class="stats-summary">
                ...
                <div class="stat-box">
                    ...
                    <div class="content">
                        <h4>Aktif</h4>
                        <div class="value">
                            <?= count(array_filter($divisions, fn($bu) => !($bu['is_deleted'] === true || $bu['is_deleted'] === 't' || $bu['is_deleted'] === '1' || $bu['is_deleted'] === 1))) ?>
                        </div>
                    </div>
                </div>
                <div class="stat-box">
                        ...
                        <tbody>
                            <?php if (!empty($divisions)): ?>
                                ...
                                <tr>
                                    ...
                                    <td><code><?= esc($bu['id']) ?></code></td>
                                    <td><strong><?= esc($bu['name']) ?></strong></td>
                                    <td><span class="code-badge"><?= esc($bu['code']) ?></span></td>
                                    <td style="color:#666; font-size:.9em; max-width:260px;">
                                        <?= esc($bu['description'] ?? '-') ?>
                                    </td>
                                    <td>
                                        <?php
                                        $aliases = is_array($bu['aliases']) ? $bu['aliases'] : [];
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <button class="btn btn-warning btn-sm btn-edit"
                                                data-id="<?= esc($bu['id']) ?>"
                                                data-name="<?= esc($bu['name']) ?>"
                                                data-code="<?= esc($bu['code']) ?>"
                                                data-description="<?= esc($bu['description'] ?? '') ?>"
                                                data-aliases="<?= esc(json_encode(is_array($bu['aliases']) ? $bu['aliases'] : [])) ?>">
                                                ✏️ Edit
                                            </button>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="7" style="text-align:center; padding:40px; color:#999;">
                                        Belum ada data business unit. Klik "Tambah" untuk menambahkan.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
    ...
    <!-- ═══════════════════════════════════════════════
         Add / Edit Modal
    ═══════════════════════════════════════════════ -->
    <div id="divModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.5); z-index:9999; align-items:center; justify-content:center;">
        <div class="modal-content" style="background:#fff; border-radius:12px; width:100%; max-width:520px; margin:auto;">
            <div class="modal-header">
                <h2 id="divModalTitle">➕ Tambah Division</h2>
                <button class="modal-close" id="btnCloseDivModal">&times;</button>
            </div>
            <div class="modal-body">
                <form id="divForm" method="POST" action="">
                    <?= csrf_field() ?>
                    <input type="hidden" id="oldDivId" name="old_id">

                    <div class="form-group">
                        <label>ID <span class="required">*</span></label>
                        <input type="text" class="form-control" id="divId" name="id" placeholder="BU-BDM-...">
                        <div id="idError" style="color:#dc3545; font-size:.85em; margin-top:4px; display:none;"></div>
                    </div>

                    <div class="form-row">
                        <div class="form-group">
                            <label>Nama Division <span class="required">*</span></label>
                            <input type="text" class="form-control" id="divName" name="name" placeholder="contoh: Mining">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="divCode" name="code" placeholder="contoh: BDM-MNG" style="text-transform:uppercase;">
                            <div id="codeError" style="color:#dc3545; font-size:.85em; margin-top:4px; display:none;"></div>
                        </div>
                    </div>

                    <div class="form-group">
                        <label>Deskripsi</label>
                        <textarea class="form-control" id="divDescription" name="description" rows="3" placeholder="Deskripsi division (opsional)"></textarea>
                    </div>
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary" id="btnCancelDivModal">Batal</button>
                <button class="btn btn-primary" id="btnSaveDivModal">
    ...
    <style>
        ...
        .code-badge      { font-family:monospace; background:#e8f0fe; color:#1a56db; padding:3px 10px; border-radius:6px; font-size:.82em; font-weight:600; letter-spacing:.03em; }
        #divCode          { text-transform: uppercase; }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const divTitle      = document.getElementById('divModalTitle');
        const divId         = document.getElementById('divId');
        const oldDivId         = document.getElementById('oldDivId');
        ...
        document.getElementById('btnTambah').addEventListener('click', function () {
            divTitle.textContent     = '➕ Tambah Division';
            oldDivId.value              = '';
            divId.value              = '';
            divName.value            = '';
            divCode.value            = '';
            ...
            currentAliases             = [];
            renderAliasList();
            divForm.action           = BASE_URL + 'employees/division/store';
        ...
        document.querySelectorAll('.btn-edit').forEach(function (btn) {
            btn.addEventListener('click', function () {
                divTitle.textContent     = '✏️ Edit Division';
                oldDivId.value              = this.dataset.id;
                divId.value              = this.dataset.id;
                divName.value            = this.dataset.name;
                divCode.value            = this.dataset.code;
                divDesc.value            = this.dataset.description;
                currentAliases                = JSON.parse(this.dataset.aliases || '[]');
                renderAliasList();              // ← populate the list UI
                divForm.action           = BASE_URL + 'employees/division/update/' + this.dataset.id;
                nameError.style.display = 'none';
                codeError.style.display = 'none';
                showModal(divModal);
            });
        ...
        // ── Save ─────────────────────────────────────────────
        document.getElementById('btnSaveDivModal').addEventListener('click', function () {
            ...
            divForm.appendChild(aliasInput);

            let valid = true;

            if (!divName.value.trim()) {
                nameError.textContent   = '⚠️ Nama business unit wajib diisi.';
            ...
            if (!divCode.value.trim()) {
                codeError.textContent   = '⚠️ Kode wajib diisi.';
        ...
        divCode.addEventListener('input', function () {
        ...
        document.getElementById('btnCloseDivModal').addEventListener('click',  function () { hideModal(divModal); });
        document.getElementById('btnCancelDivModal').addEventListener('click', function () { hideModal(divModal); });
        divModal.addEventListener('click', function (e) { if (e.target === divModal) hideModal(divModal); });

        // ── Toggle Buttons ────────────────────────────────────
        document.querySelectorAll('.btn-toggle').forEach(function (btn) {
            btn.addEventListener('click', function () {
                toggleForm.action      = BASE_URL + 'employees/division/toggle/' + this.dataset.id;
                toggleName.textContent = this.dataset.name;

                if (this.dataset.action === 'disable') {
                    toggleTitle.textContent      = '⏸️ Konfirmasi Nonaktifkan';
                    toggleMessage.textContent    = 'Apakah Anda yakin ingin menonaktifkan business unit:';
                    toggleConfirm.className      = 'btn btn-danger';
                    toggleConfirm.textContent    = '⏸️ Nonaktifkan';
                } else {
                    toggleTitle.textContent      = '✅ Konfirmasi Aktifkan';
                    toggleMessage.textContent    = 'Apakah Anda yakin ingin mengaktifkan kembali business unit:';
