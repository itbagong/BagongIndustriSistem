...
<?= $this->section('content') ?>
                ...
                <form method="GET" action="" id="searchForm" style="display:flex; gap:12px; align-items:flex-end;">
                    <div class="form-group" style="flex:1; margin:0;">
                        ...
                        <input type="text" class="form-control" name="search" id="searchInput"
                               placeholder="Cari name, description..."
                               value="<?= esc($search) ?>">
                    </div>
                        ...
                        <tbody>
                            <?php if (!empty($departments)): ?>
                                <?php foreach ($departments as $i => $dept):
                                    $isDeleted = ($dept['is_deleted'] === true || $dept['is_deleted'] === 't' || $dept['is_deleted'] === '1' || $dept['is_deleted'] === 1);
                                ?>
                                <tr>
                                    <td><?= ($pager->getCurrentPage() - 1) * $perPage + $i + 1 ?></td>
                                    <td><code><?= esc($dept['id']) ?></code></td>
                                    <td><strong><?= esc($dept['name']) ?></strong></td>
                                    <td style="color:#666; font-size:.9em;">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="6" style="text-align:center; padding:40px; color:#999;">
                                        Belum ada data department. Klik "Tambah" untuk menambahkan.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
    ...
    <!-- ═══════════════════════════════════════════════
         Add / Edit Modal
    ═══════════════════════════════════════════════ -->
    <div id="deptModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.5); z-index:9999; align-items:center; justify-content:center;">
        <div class="modal-content" style="background:#fff; border-radius:12px; width:100%; max-width:520px; margin:auto;">
            <div class="modal-header">
                <h2 id="deptModalTitle">➕ Tambah Department</h2>
            ...
            <div class="modal-body">
                <form id="deptForm" method="POST" action="">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="deptName" name="name" placeholder="contoh: Human Capital & General Services">
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary" id="btnCancelDeptModal">Batal</button>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('btnCloseDeptModal').addEventListener('click',  function () { hideModal(deptModal); });
        document.getElementById('btnCancelDeptModal').addEventListener('click', function () { hideModal(deptModal); });
        deptModal.addEventListener('click', function (e) { if (e.target === deptModal) hideModal(deptModal); });
        ...
        document.querySelectorAll('.btn-toggle').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                toggleForm.action      = BASE_URL + 'employees/department/toggle/' + id;
                toggleName.textContent = name;

                if (action === 'disable') {
                    toggleTitle.textContent      = '⏸️ Konfirmasi Nonaktifkan';
                    toggleMessage.textContent    = 'Apakah Anda yakin ingin menonaktifkan department:';
                    toggleConfirm.className      = 'btn btn-danger';
                    toggleConfirm.textContent    = '⏸️ Nonaktifkan';
                } else {
                    toggleTitle.textContent      = '✅ Konfirmasi Aktifkan';
                    toggleMessage.textContent    = 'Apakah Anda yakin ingin mengaktifkan kembali department:';
