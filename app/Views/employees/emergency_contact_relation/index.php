...
<?= $this->section('content') ?>
        ...
        <main class="content">
            ...
            <div class="page-header">
                <div class="page-title">
                    ...
                    <h1>Manajemen Emergency Contact Relation</h1>
                </div>
                <div class="header-actions">
                    <button class="btn btn-primary" id="btnTambah">
                        <span class="btn-icon">➕</span>
                        Tambah Emergency Contact Relation
                    </button>
                </div>
            </div>
            ...
            <div class="stats-summary">
                <div class="stat-box">
                    <span class="icon">📋</span>
                    <div class="content">
                        <h4>Total Emergency Contact Relation</h4>
                ...
                <div class="stat-box">
                    ...
                    <div class="content">
                        <h4>Aktif</h4>
                        <div class="value">
                            <?= count(array_filter($emergencyContactRelations, fn($es) => !($es['is_deleted'] === true || $es['is_deleted'] === 't' || $es['is_deleted'] === '1' || $es['is_deleted'] === 1))) ?>
                        </div>
                    </div>
                </div>
                <div class="stat-box">
            ...
            <div class="table-card">
                <div class="table-header">
                    <h3>Daftar Emergency Contact Relation</h3>
                ...
                <div class="table-responsive">
                    <table class="employee-table">
                        ...
                        <tbody>
                            <?php if (!empty($emergencyContactRelations)): ?>
                                <?php foreach ($emergencyContactRelations as $i => $es):
                                    $isDeleted = (
                                        $es['is_deleted'] === true ||
                                        $es['is_deleted'] === 't'  ||
                                        $es['is_deleted'] === '1'  ||
    ...
    <!-- ═══════════════════════════════════════════════
         Add / Edit Modal
    ═══════════════════════════════════════════════ -->
    <div id="ecrModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.5); z-index:9999; align-items:center; justify-content:center;">
        <div class="modal-content" style="background:#fff; border-radius:12px; width:100%; max-width:520px; margin:auto;">
            <div class="modal-header">
                <h2 id="ecrModalTitle">➕ Tambah Emergency Contact Relation</h2>
                <button class="modal-close" id="btnCloseEcrModal">&times;</button>
            </div>
            <div class="modal-body">
                <form id="ecrForm" method="POST" action="">
                    <?= csrf_field() ?>
                    <input type="hidden" id="ecrId" name="id">

                    <div class="form-group">
                        <label>Nama <span class="required">*</span></label>
                        <input type="text" class="form-control" id="ecrName" name="name" placeholder="contoh: Permanen, PKWT, Trainee">
                        <div id="nameError" style="color:#dc3545; font-size:.85em; margin-top:4px; display:none;"></div>
                    </div>

                    <div class="form-group">
                        <label>Deskripsi</label>
                        <textarea class="form-control" id="ecrDescription" name="description" rows="3" placeholder="Deskripsi relasi kontak darurat (opsional)"></textarea>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button class="btn btn-secondary" id="btnCancelEcrModal">Batal</button>
                <button class="btn btn-primary" id="btnSaveEcrModal">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // ── Elements ──────────────────────────────────────────
        const ecrModal      = document.getElementById('ecrModal');
        const ecrForm       = document.getElementById('ecrForm');
        const ecrTitle      = document.getElementById('ecrModalTitle');
        const ecrId         = document.getElementById('ecrId');
        const ecrName       = document.getElementById('ecrName');
        const ecrDesc       = document.getElementById('ecrDescription');
        ...
        document.getElementById('btnTambah').addEventListener('click', function () {
            ecrTitle.textContent     = '➕ Tambah Emergency Contact Relation';
            ecrId.value              = '';
            ecrName.value            = '';
            ecrDesc.value            = '';
            ecrForm.action           = BASE_URL + 'employees/emergency-contact-relation/store';
        ...
        document.querySelectorAll('.btn-edit').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ecrTitle.textContent     = '✏️ Edit Emergency Contact Relation';
                ecrId.value              = this.dataset.id;
                ecrName.value            = this.dataset.name;
                ecrDesc.value            = this.dataset.description;
                ecrForm.action           = BASE_URL + 'employees/emergency-contact-relation/update/' + this.dataset.id;
                nameError.style.display = 'none';
                showModal(ecrModal);
            });
        });
        ...
        // ── Save ─────────────────────────────────────────────
        document.getElementById('btnSaveEcrModal').addEventListener('click', function () {
            if (!ecrName.value.trim()) {
        ...
        document.getElementById('btnCloseEcrModal').addEventListener('click',  function () { hideModal(ecrModal); });
        document.getElementById('btnCancelEcrModal').addEventListener('click', function () { hideModal(ecrModal); });
        ecrModal.addEventListener('click', function (e) { if (e.target === ecrModal) hideModal(ecrModal); });

        // ── Toggle Buttons ────────────────────────────────────
        document.querySelectorAll('.btn-toggle').forEach(function (btn) {
            btn.addEventListener('click', function () {
                toggleForm.action      = BASE_URL + 'employees/emergency-contact-relation/toggle/' + this.dataset.id;
