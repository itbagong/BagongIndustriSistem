...
<?= $this->section('content') ?>
    ...
    <!-- ═══════════════════════════════════════════════
         Add / Edit Modal
    ═══════════════════════════════════════════════ -->
    <div id="religionModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.5); z-index:9999; align-items:center; justify-content:center;">
        <div class="modal-content" style="background:#fff; border-radius:12px; width:100%; max-width:520px; margin:auto;">
            <div class="modal-header">
                <h2 id="religionModalTitle">➕ Tambah Religion</h2>
                <button class="modal-close" id="btnCloseReligionModal">&times;</button>
            </div>
            <div class="modal-body">
                <form id="religionForm" method="POST" action="">
                    <?= csrf_field() ?>
                    <input type="hidden" id="oldReligionId" name="old_id">

                    <div class="form-group">
                        <label>ID <span class="required">*</span></label>
                        <input type="text" class="form-control" id="religionId" name="id" placeholder="REL-BDM-...">
                        <div id="idError" style="color:#dc3545; font-size:.85em; margin-top:4px; display:none;"></div>
                    </div>

                    <div class="form-group">
                        <label>Nama <span class="required">*</span></label>
                        <input type="text" class="form-control" id="religionName" name="name" placeholder="contoh: Protestan, Islam...">
                        <div id="nameError" style="color:#dc3545; font-size:.85em; margin-top:4px; display:none;"></div>
                    </div>

                    <div class="form-group">
                        <label>Deskripsi</label>
                        <textarea class="form-control" id="religionDescription" name="description" rows="3" placeholder="Deskripsi golongan (opsional)"></textarea>
                    </div>
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary" id="btnCancelReligionModal">Batal</button>
                <button class="btn btn-primary" id="btnSaveReligionModal">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const religionForm       = document.getElementById('religionForm');
        const religionTitle      = document.getElementById('religionModalTitle');
        const religionId         = document.getElementById('religionId');
        const oldReligionId      = document.getElementById('oldReligionId');
        const religionName       = document.getElementById('religionName');
        const religionDesc       = document.getElementById('religionDescription');
        ...
        document.getElementById('btnTambah').addEventListener('click', function () {
            religionTitle.textContent     = '➕ Tambah Religion';
            oldReligionId.value           = '';
            religionId.value              = '';
            religionName.value            = '';
            religionDesc.value            = '';
            ...
            religionForm.action           = BASE_URL + 'employees/religion/store';
            nameError.style.display = 'none';
            showModal(religionModal);
        });

        // ── Edit ─────────────────────────────────────────────
        document.querySelectorAll('.btn-edit').forEach(function (btn) {
            btn.addEventListener('click', function () {
                religionTitle.textContent     = '✏️ Edit Religion';
                oldReligionId.value           = this.dataset.id;
                religionId.value              = this.dataset.id;
                religionName.value            = this.dataset.name;
                religionDesc.value            = this.dataset.description;
                currentAliases                = JSON.parse(this.dataset.aliases || '[]');
                renderAliasList();              // ← populate the list UI
                religionForm.action           = BASE_URL + 'employees/religion/update/' + this.dataset.id;
                nameError.style.display = 'none';
                showModal(religionModal);
            });
        });
        ...
        // ── Save ─────────────────────────────────────────────
        document.getElementById('btnSaveReligionModal').addEventListener('click', function () {
            ...
            religionForm.appendChild(aliasInput);

            religionForm.submit();
        });

        // ── Close Add/Edit Modal ──────────────────────────────
        document.getElementById('btnCloseReligionModal').addEventListener('click',  function () { hideModal(religionModal); });
        document.getElementById('btnCancelReligionModal').addEventListener('click', function () { hideModal(religionModal); });
        religionModal.addEventListener('click', function (e) { if (e.target === religionModal) hideModal(religionModal); });

        // ── Toggle Buttons ────────────────────────────────────
        document.querySelectorAll('.btn-toggle').forEach(function (btn) {
            btn.addEventListener('click', function () {
                toggleForm.action      = BASE_URL + 'employees/religion/toggle/' + this.dataset.id;
