...
class DepartmentModel extends Model
{
    ...
    // INSERT with aliases
    // -------------------------------------------------------
    public function insertDept(array $data): void
    {
        $aliases = $this->toPostgresArray($data['aliases'] ?? []);

        $this->db->query(
            'INSERT INTO departments (id, name, description, aliases, is_deleted)
             VALUES (?, ?, ?, ?, false)',
    ...
    // -------------------------------------------------------
    public function updateDept(string $oldId, array $data): void
    {
        ...
        if ($newId !== $oldId) {
            ...
            $this->db->query('DELETE FROM departments WHERE id = ?', [$oldId]);

        } else {
            $this->db->query(
                'UPDATE departments SET name=?, description=?, aliases=? WHERE id=?',
