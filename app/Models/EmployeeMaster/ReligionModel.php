...
class ReligionModel extends Model
{
    ...
    public function generateId(): string
    {
        ...
        return 'REL-BDM-' . str_pad($num + 1, 4, '0', STR_PAD_LEFT);
    }
    ...
    // INSERT with aliases
    // -------------------------------------------------------
    public function insertReligion(array $data): void
    {
        $aliases = $this->toPostgresArray($data['aliases'] ?? []);

        $this->db->query(
            'INSERT INTO religions (id, name, description, aliases, is_deleted)
             VALUES (?, ?, ?, ?, false)',
    ...
    // -------------------------------------------------------
    public function updateReligion(string $oldId, array $data): void
    {
        ...
        if ($newId !== $oldId) {
            ...
            $this->db->query(
                'INSERT INTO religions (id, name, description, aliases, is_deleted)
                 SELECT ?, ?, ?, ?, is_deleted FROM religions WHERE id = ?',
                [$newId, $data['name'], $data['description'], $aliases, $oldId]
            );

            // 2. Re-point every employee that referenced the old site_id
            $this->db->query(
                'UPDATE employees SET religion_id = ? WHERE religion_id = ?',
                [$newId, $oldId]
            );

            // 3. Remove the old row
            $this->db->query('DELETE FROM religions WHERE id = ?', [$oldId]);

        } else {
            $this->db->query(
                'UPDATE religions SET name=?, description=?, aliases=? WHERE id=?',
        ...
        if (!$this->db->transStatus()) {
            throw new \RuntimeException('Religion update transaction failed.');
        }
    }
}