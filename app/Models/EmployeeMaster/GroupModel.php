...
class GroupModel extends Model
{
    ...
    public function generateId(): string
    {
        ...
        return 'GRO-BDM-' . str_pad($num + 1, 4, '0', STR_PAD_LEFT);
    }
    ...
    // INSERT with aliases
    // -------------------------------------------------------
    public function insertGroup(array $data): void
    {
    ...
    // -------------------------------------------------------
    public function updateGroup(string $oldId, array $data): void
    {
        ...
        if ($newId !== $oldId) {
            ...
            $this->db->query(
                'INSERT INTO groups (id, name, description, aliases, is_deleted)
                 SELECT ?, ?, ?, ?, is_deleted FROM groups WHERE id = ?',
                [$newId, $data['name'], $data['description'], $aliases, $oldId]
            );

            // 2. Re-point every employee that referenced the old site_id
            $this->db->query(
                'UPDATE employees SET job_position_id = ? WHERE job_position_id = ?',
                [$newId, $oldId]
            );

            // 3. Remove the old row
            $this->db->query('DELETE FROM groups WHERE id = ?', [$oldId]);

        } else {
            $this->db->query(
                'UPDATE groups SET name=?, description=?, aliases=? WHERE id=?',
        ...
        if (!$this->db->transStatus()) {
            throw new \RuntimeException('Group update transaction failed.');
        }
    }
}